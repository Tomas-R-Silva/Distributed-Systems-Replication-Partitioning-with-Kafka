...
public class Discovery {
	...
	public static void announce(String serviceName, String domain, String serviceURI) {
		byte[] announceBytes = String.format("%s@%s%s%s", serviceName, domain, DELIMITER, serviceURI).getBytes();
		DatagramPacket announcePkt = new DatagramPacket(announceBytes, announceBytes.length, DISCOVERY_ADDR);

		new Thread(() -> {
			for (;;) {
				try {
					getInstance().ms.send(announcePkt);
	 ...
	 */
	private Discovery(InetSocketAddress addr)
